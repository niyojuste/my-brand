...
fetch(`https://juste-my-brand.herokuapp.com/api/posts/${article}`, reqHeader)
	...
	.then((post) => {
		...
		likeSVG.addEventListener('click', function (e) {
			...
			fetch(
				`https://juste-my-brand.herokuapp.com/api/posts/${article}/likes`,
				likeHeader
			)
				.then((response) => response.json())
				.then((result) => {
...
fetch(
	`https://juste-my-brand.herokuapp.com/api/posts/${article}/comments`,
	reqHeader
)
	.then((response) => response.json())
	.then((comments) => {
...
if (!localStorage.getItem('active')) {
	textarea.value = 'Login to comment'
